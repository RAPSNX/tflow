...
migration or rewrite. Within one project, an existing `git`-typed
record's label converges to `git`, and every `git`-typed record after
the first (stored order) converges to `terminal`. Present types must be
