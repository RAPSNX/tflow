...
func (m model) updateNormal(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "n":
		m.mode = inputNew
		m.status = "New: np project, nt terminal, nk k9s, nc codex, na add current temp."
		return m, nil
	case "c":
...
func (m model) renderHeader(width int) string {
	...
	right := mutedStyle.Render(fmt.Sprintf("%d projects  %d sections", len(m.projects), m.visibleSessionCount()))
...
func (m model) renderFooter(width int) string {
	lines := []string{}
	if m.mode == inputNew {
		lines = append(lines, hintStyle.Render("new: [p] project  [t] terminal  [k] k9s  [c] codex  [a] add temp  [esc] cancel"))
	}
